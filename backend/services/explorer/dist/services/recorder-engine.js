...
class IntelligentNavigator {
    ...
    crawlPages = [];
    pageMap = new Map();
    urlMap = new Map(); // path -> full URL
    constructor(crawlData, storyboard, logger) {
     ...
     */
    resolveNavigationUrl(action, baseUrl) {
    ...
    smartGuessUrl(action, baseUrl) {
        const targetPath = action.value || '/';
        const description = action.description.toLowerCase();
        // If description mentions specific page types, try to find them
        if (description.includes('dashboard') || description.includes('app')) {
            const dashboardPage = this.crawlPages.find(p => p.url.includes('/dashboard') || p.url.includes('/app') || p.title.toLowerCase().includes('dashboard'));
            if (dashboardPage)
                return dashboardPage.url;
        }
        if (description.includes('getting started') || description.includes('get started')) {
            const startPage = this.crawlPages.find(p => p.url.includes('/start') || p.url.includes('/getting-started') ||
     ...
     */
    findBestElement(action, currentPageUrl) {
        if (!this.crawlData) {
    ...
    findMatchingElements(page, searchText) {
...
export class RecorderEngine {
    ...
    async record(storyboard, jobId, auth) {
        ...
        try {
            ...
            for (let i = 0; i < storyboard.scenes.length; i++) {
                ...
                try {
                    const result = await this.recordScene(scene, sceneNumber, storyboard.baseUrl, sceneDir, traceDir, auth, storyboard.globalBlurSelectors);
    ...
    async recordScene(scene, sceneNumber, baseUrl, sceneDir, traceDir, auth, globalBlurSelectors) {
    ...
    async executeScene(page, scene, baseUrl, logger) {
        ...
        if (this.intelligentNavigator) {
            ...
            try {
                ...
                logger.warn('Could not extract page elements for filtering', { error: error.message });
            }
            ...
            scene.actions = scene.actions.filter(action => {
                if (action.type === 'click' && action.locator?.value) {
                    ...
                    const hasMatchingElement = currentPageElements.some(element => element.toLowerCase().includes(targetText) || targetText.includes(element.toLowerCase()));
    ...
    async executeAction(page, action, baseUrl, logger) {
    ...
    async clickElement(page, locator, timeout) {
        ...
        try {
            ...
            throw new Error(`FATAL CLICK FAILURE: Cannot find element "${locator.value}" on page "${pageTitle}" (${currentUrl}). Available clickable elements: [${availableButtons.slice(0, 10).join(', ')}]. Original error: ${error.message}`);
        }
    }
    async typeInElement(page, locator, value, timeout) {
        const strategies = [
    ...
    async assertElement(page, locator, expectedValue, timeout = 5000) {
        try {
    ...
    findElementByLocator(page, locator) {
        switch (locator.type) {
            case 'role':
    ...
    async tryStrategies(strategies, operation) {
    ...
    async applyBlurOverlays(page, selectors) {
        const blurCSS = `
    ...
    async smartNavigate(page, action, baseUrl, logger, intelligentNavigator) {
    ...
    async findAndRenameVideo(sceneDir, sceneNumber) {
