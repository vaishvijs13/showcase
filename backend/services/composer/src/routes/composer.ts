...
composerRouter.post('/music', async (req: Request, res: Response) => {
  try {
    const { jobId, style, duration, outputPath } = req.body as MusicRequest;
...
// Compose final video
composerRouter.post('/compose', async (req: Request, res: Response) => {
  try {
    const { jobId, storyboardPath, scenePaths, musicPath, config } = req.body as ComposeRequest;
    ...
    const resolvedScenePaths: string[] = [];
...
composerRouter.post('/test', async (req: Request, res: Response) => {
  try {
    const sunoClient = new SunoClient(process.env.SUNO_API_KEY!);
...
composerRouter.get('/styles', async (req: Request, res: Response) => {
  try {
    const sunoClient = new SunoClient(process.env.SUNO_API_KEY!);
