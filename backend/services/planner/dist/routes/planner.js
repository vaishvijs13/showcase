...
plannerRouter.post('/generate', async (req, res) => {
    try {
        const { jobConfig, featuresPath, crawlPath } = req.body;
        ...
        // Generate storyboard
        const storyboard = await storyboardGenerator.generateStoryboard(jobConfig, features, crawlSummary);
...
plannerRouter.post('/test', async (req, res) => {
    try {
        const cerebrasClient = new CerebrasClient(process.env.CEREBRAS_API_KEY);
...
async function loadFeatureCandidates(featuresPath) {
    try {
...
async function loadCrawlSummary(crawlPath) {
    try {
        // Check if path is absolute, if not make it relative to explorer storage
...
function createEmptyCrawlSummary() {
    return {
        baseUrl: '',
        pages: [],
        totalPages: 0,
